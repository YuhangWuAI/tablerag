...
for task in "${task_name[@]}"; do
    for sampling_method in "${sampling_methods[@]}"; do
        ...
        python run.py  --gpt_model gpt-3.5-turbo --total_tokens 160000 -t $task -r $sampling_method --experiment_name table_sampling --load_local_dataset --whether_cot --azure_blob --table_token_limit_portion 70 --augmentation_token_limit_portion 0
